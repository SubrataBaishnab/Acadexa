...
const EligibilityPage = () => {
    ...
    if (result && result.status === 'Approved') {
      const fetchSupervisors = async () => {
        try {
          ...
          let fetchedData = response.data.data || response.data.supervisors || response.data; 
          
          if (Array.isArray(fetchedData) && fetchedData.length > 0) {
             const mappedProfs = fetchedData.map(prof => ({
                 supervisor_id: prof._id || prof.supervisor_id,
  ...
  const handleModule2Submit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/registration/team-setup`, {
        student_id: studentId, thesis_title: thesisTitle, group_members: memberArray
      });
      const response = await axios.put(`${API_URL}/registration/assign-supervisor`, {
...
export default EligibilityPage;import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { API_URL } from '../services/apiService';
